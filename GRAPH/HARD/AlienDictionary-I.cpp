...
string findOrder(string dict[], int N, int K) {
  ...
  for(int i = 0; i < N-1; i++) {
      ...
      for(int j = 0; j < len; j++) {
          // * If unequal charcter found means, we found 1 order of alphabets,
          // * which we can be certain that something appears before something
          // * So, create a directed edge between those alphabets.
          if(str1[j] != str2[j]){ 
